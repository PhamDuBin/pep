  ...
  return (
    <nav className="h-[94px] bg-[#ffffff] shadow-[0px_4px_10px_rgba(0,0,0,0.05)] flex">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          className={`flex-1 flex items-center justify-center gap-[10px] p-[25px_10px] transition-all duration-200 border-b border-t-0 border-l-0 border-r-0 cursor-pointer bg-transparent ${tab.isActive ? "border-b-[#066a9e]" : "border-b-[#b9b9b9]"
            } ${tab.isDisabled
              ? "cursor-not-allowed"
        ...
        >
          ...
          {tab.icon === "kick" && (
            ...
            >
              <path
                className={`transition-[fill] duration-200 ${tab.isActive ? "fill-[#066a9e]" : "fill-[#808080]"
                  }`}
          ...
          <div className="flex flex-col items-center">
            <span
              className={`text-[16px] font-bold text-center transition-colors duration-200 ${tab.isActive ? "text-[#066a9e]" : "text-[#808080]"
                }`}
            >
              {tab.label}
            </span>
            <span
              className={`text-[16px] text-center transition-colors duration-200 ${tab.isActive ? "text-[#066a9e] font-bold" : "text-[#808080]"
                }`}
            >
              {tab.subLabel}
            </span>
