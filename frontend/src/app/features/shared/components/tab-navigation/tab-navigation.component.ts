...
@Component({
    ...
    template: `
    <nav class="h-[94px] bg-white shadow-tab flex">
      @for (tab of tabs; track tab.id) {
        <button
          class="flex-1 flex items-center justify-center gap-[10px] px-[10px] py-[25px] transition-all duration-200 border-b group"
          [class.border-primary]="tab.isActive"
          [class.border-border-gray]="!tab.isActive"
          [class.hover:bg-primary]="!tab.isDisabled"
          [class.cursor-pointer]="!tab.isDisabled"
          [class.cursor-not-allowed]="tab.isDisabled"
          [disabled]="tab.isDisabled"
          (click)="selectTab(tab)"
        >
          <!-- Kick Icon -->
          @if (tab.icon === 'kick') {
            <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="transition-colors duration-200">
              <path 
                class="transition-colors duration-200"
                [class.fill-primary]="tab.isActive && !isHovered(tab.id)"
                [class.fill-text-gray]="!tab.isActive && !isHovered(tab.id)"
                [class.group-hover:fill-white]="!tab.isDisabled"
          ...
          <!-- Carry Icon -->
          @if (tab.icon === 'carry') {
            <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" class="transition-colors duration-200">
              <path 
                class="transition-colors duration-200"
                [class.fill-primary]="tab.isActive && !isHovered(tab.id)"
                [class.fill-text-gray]="!tab.isActive && !isHovered(tab.id)"
          ...
          <div class="flex flex-col items-center">
            <span 
              class="text-[16px] font-bold text-center transition-colors duration-200"
              [class.text-primary]="tab.isActive"
              [class.text-text-gray]="!tab.isActive"
              [class.group-hover:text-white]="!tab.isDisabled"
            >{{ tab.label }}</span>
            <span 
              class="text-[16px] text-center transition-colors duration-200"
              [class.text-primary]="tab.isActive"
              [class.text-text-gray]="!tab.isActive"
