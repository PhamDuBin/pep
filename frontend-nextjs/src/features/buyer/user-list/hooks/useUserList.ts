...
import type { InviteMemberModalState } from "@/components/ui/Modal/InviteMemberModal/InviteMemberModal";
import type { PermissionChangeModalState } from "../components/PermissionChangeModal/PermissionChangeModal";
import type { DeleteConfirmModalState } from "@/components/ui/Modal/DeleteConfirmModal/DeleteConfirmModal";
...
export function useUserList() {
  ...
  // Modal states
  const [inviteModalState, setInviteModalState] = useState<InviteMemberModalState>("form");
  const [permissionModalState, setPermissionModalState] = useState<PermissionChangeModalState>("select");
  const [deleteModalState, setDeleteModalState] = useState<DeleteConfirmModalState>("confirm");
  ...
  const handleConfirmDelete = useCallback(async () => {
    setIsDeleting(true);
    try {
      const userIdsToDelete = users.filter((u) => u.isSelected).map((u) => u.id);
