...
import type { EmailChangeModalState } from "@/components/ui/Modal/EmailChangeModal/EmailChangeModal";
...
export function useMyPage(): UseMyPageReturn {
  ...
  const [emailModalState, setEmailModalState] = useState<EmailChangeModalState>("email-change");
  ...
  const handleSendEmailChange = useCallback(async (newEmail: string, confirmEmail: string) => {
