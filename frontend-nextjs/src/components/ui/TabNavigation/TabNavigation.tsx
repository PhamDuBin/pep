...
export function TabNavigation({ tabs: initialTabs, onTabChange }: TabNavigationProps) {
  const [tabs, setTabs] = useState(initialTabs);
  const [hoveredTabId, setHoveredTabId] = useState<string | null>(null);

  const isHovered = useCallback(
    (tabId: string) => hoveredTabId === tabId,
    [hoveredTabId]
  );
            ...
            >
              <path
                className={`${styles.iconPath} ${
                  tab.isActive && !isHovered(tab.id) ? styles.iconActive : ""
                } ${!tab.isActive && !isHovered(tab.id) ? styles.iconInactive : ""}`}
          ...
          <div className={styles.labelContainer}>
            <span
              className={`${styles.label} ${
                tab.isActive ? styles.labelActive : styles.labelInactive
              }`}
            >
              {tab.label}
            </span>
            <span
              className={`${styles.subLabel} ${
                tab.isActive ? styles.subLabelActive : styles.subLabelInactive
              }`}
            >
              {tab.subLabel}
            </span>
