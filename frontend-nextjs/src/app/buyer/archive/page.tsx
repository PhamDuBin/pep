...
export default function ArchivePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setProjects(MOCK_ARCHIVE_PROJECTS);
      setIsLoading(false);
    };
    loadData();
  }, []);
  ...
  const selectedFilterLabel = useMemo(() => {
    const option = PROJECT_FILTER_OPTIONS.find((o) => o.id === selectedFilter);
  ...
  const handleSortSelect = useCallback((value: "asc" | "desc") => {
    setSortOrder(value);
    setShowSortDropdown(false);
  }, []);

  const handleMenuClick = useCallback((projectId: string) => {
  ...
  const handleContextAction = useCallback(
    (projectId: string, action: string) => {
      ...
      setOpenContextMenuId(null);
      // Handle actions: star, unstar, restore, delete
    },
    []
  );
  ...
  const menuItems = [
    { action: "star", label: " ダウンロード " },
    { action: "restore", label: " 名前を変更 " },
    { action: "delete", label: " お気に入り " },
    { action: "favorite", label: " 削除 " },
  ];
          ...
          <div className={styles.archiveFilter}>
                  ...
                  {PROJECT_FILTER_OPTIONS.map((option) => (
                    <button
                      ...
                      className={`${styles.dropdownItem} ${
                        option.id === selectedFilter ? styles.selected : ""
                      }`}
                      type="button"
            ...
            <div className={styles.dropdownContainer} ref={sortRef}>
              <button
                ...
                type="button"
              >
                <span className={styles.dropdownLabel}>作成日</span>
                <svg
                  className={styles.dropdownIcon}
                  width="10"
                  height="10"
                ...
                <div className={`${styles.dropdownMenu} ${styles.sortMenu}`}>
                  {SORT_OPTIONS.map((option) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        handleSortSelect(option.value as "asc" | "desc")
                      }
                    >
                      ...
                      <span
                        className={
                          option.value === sortOrder ? styles.selectedText : ""
                        }
                      >
                        {option.label}
                      </span>
        ...
        {isLoading ? (
          ...
          <div className={styles.projectsList}>
            ...
            <div className={styles.listHeader}>
              <span className={styles.headerProjectName}>プロジェクト名</span>
              <div className={styles.headerInfo}>
                <span className={styles.headerAuthor}>作成者</span>
                <span className={styles.headerDate}>作成日</span>
                    ...
                    <div className={styles.infoSection}>
                      <div className={styles.infoContent}>
                        <div className={styles.authorCell}>
                          ...
                          <span className={styles.authorNameList}>
                            {project.authorName}
                          </span>
                        </div>
                        <div className={styles.dateCell}>
                          <span className={styles.createdDateList}>
                            {project.createdAt}
                          </span>
                        </div>
                      </div>
                      <button
                        className={styles.menuBtnList}
                        onClick={() => handleMenuClick(project.id)}
                        type="button"
                      >
                        <svg
