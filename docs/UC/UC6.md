# UC06: Project Plan Draft Creation (AI Chat) / プロジェクト計画書草案の作成（AIチャット）

```mermaid
sequenceDiagram
    autonumber
    actor BU as BuyerUser<br/>(Admin/Member common / Admin/Member共通)
    participant BWeb as Buyer PEP Web<br/>(Project Plan Creation Screen / プロジェクト計画書作成画面)
    participant API as PEP API<br/>(Backend / バックエンド)
    participant DB as PEP DB
    participant AI as OpenAI API<br/>(For Project Plan generation / プロジェクト計画書生成用)
    ...
    rect rgb(240, 248, 255)
        ...
        BWeb-->>BU: Display Project Plan creation screen / プロジェクト計画書作成画面表示
    end
    ...
    rect rgb(240, 255, 240)
        ...
        BWeb->>API: POST /project-plans/ai-suggest<br/>(conditions/overview / 条件/概要)

        API->>AI: ChatCompletion<br/>(Project Plan draft generation / プロジェクト計画書草案生成)
    ...
    rect rgb(255, 250, 240)
        ...
        loop Until Buyer is satisfied / Buyerが納得するまで
            ...
            BWeb->>API: POST /project-plans/ai-suggest<br/>(conversation history + instructions / 会話履歴＋指示)
